...
export default function LeaderSignup() {
  ...
  const onSubmit = async (data: SignupForm) => {
    try {
      setLoading(true);
      const res = await api.post('/auth/register-leader', data);
      login(res.data.token, res.data.refreshToken, res.data.user);
      toast.success('Account and team created!');
      navigate('/');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-[var(--card)] rounded-3xl border border-[var(--border)] shadow-xl p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[var(--foreground)]">Register Team</h1>
          <p className="text-sm text-[var(--muted-foreground)] mt-2">Create a new team as the Team Leader</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1.5">Team Name</label>
              <div className="relative">
                <Users className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)]" size={18} />
                <input {...register('teamName')} placeholder="Hackers"
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--muted)] text-[var(--foreground)] text-sm outline-none focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/20 transition-all" />
              </div>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <Loader2 size={18} className="animate-spin" /> : 'Create Team & Register'}
          </button>
        </form>
